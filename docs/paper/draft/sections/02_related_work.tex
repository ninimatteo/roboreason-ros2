...
for geometric grounding errors on its own. It pays for that with the one
resource imitation learning has always been bottlenecked on: paired
vision-action demonstration data, collected per embodiment, which stays
scarce and narrow next to the text/code/image-text corpora an LLM or VLM
is pretrained on. Code-generating policies that couple perception
...
reasoning error, or a motor-control error~\cite{favaliRAS}. We adopt the
decoupled architecture for both reasons at once, data efficiency and
diagnosability, since a fully end-to-end system would not permit the
diagnosis of \S\ref{sec:taxonomy} at all, only its symptoms.
...
semi-automatic adaptation). Neither it nor its companion systems paper,
RoboReason~Lab~\cite{favaliRoboReasonLab} (\S\ref{sec:harness-architecture}),
ever grounds a plan in measured depth or a calibrated camera; both are
simulation-only by design, and so structurally unable to produce the
failures \S\ref{sec:taxonomy} diagnoses.
