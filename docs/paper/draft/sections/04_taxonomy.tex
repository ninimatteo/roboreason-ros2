...
\rev{The paired \emph{release} action, however, computes how far to lift
the object before opening the gripper: \code{object\_height}, added to
\code{release\_position.z} at execution time. This computation assumes
the object was grasped at the top. That assumption is wrong by
construction whenever the pick descended mid-body, which is always.}
The result is a systematic overshoot of roughly
...
\rev{We fixed this once, in the VLM pipeline, by deriving
...
weeks later. That pipeline's prompt still asked the model to
...
height. \rev{In our VLM$\to$LLM hybrid pipeline, a VLM grounding call
first deprojects a target's pixel location to a depth-measured world
coordinate. It then hands the resulting scene to the same LLM planner
used in static-scene mode. An early version of this hybrid set the
generated scene's \code{target.position.z} directly to that
depth-measured top surface, already correct. It left the VLM's separate,
un-verified \code{size[2]} guess untouched, though.} The downstream
planner then applied its normal stacking formula, adding the guessed
height on top of an already-correct value and inflating every
stack-on-target release by however wrong that guess happened to be.
...
\emph{static} half: no two final positions occupy the same footprint. \rev{It does not address the \emph{dynamic} half. Nothing constrains
the arm's trajectory while approaching or releasing the $n$-th object
against the $(n-1)$-th already sitting in the zone. Nothing constrains
a released object's small residual motion from displacing a neighbor
afterward, either.} Both are genuinely three-dimensional, time-dependent
contact problems, not resolvable by correcting a single scalar the way
Cases 1--2 were.
