...
Table~\ref{tab:taxonomy} summarizes all six. Below, we develop the two
most instructive of the five closed cases in full, since both recurred
independently in two different planning pipelines and so best show that
the pattern is structural, not a one-off slip, then treat the sixth,
still-open bug on its own (\S\ref{sec:case-collision}), which turns out
to be the dominant failure mode in \S\ref{sec:evaluation}.
