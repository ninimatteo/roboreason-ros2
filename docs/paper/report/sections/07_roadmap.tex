...
\begin{tikzpicture}[
  ...
  \node[phase] (a) at (0,0) {
    ...
    \footnotesize
    $\bullet$ Re-verify Session 5 geometry fixes on hardware (I1)\\
    $\bullet$ Re-check camera/board calibration (I8)\\
    $\bullet$ R1: zone-bounds-aware release placement
  };
  \node[phase, right=0.9cm of a] (b) {
    \textbf{Phase B -- Model quality \& schema}\\[2pt]
    \footnotesize
    $\bullet$ R4: bounding-box scene schema\\
    $\bullet$ R5: rewrite skill/primitive prompt text\\
    $\bullet$ R3: real tray-dimension-aware placement\\
    $\bullet$ Benchmark: add VLM\_LLM condition
  };
...
\paragraph{Hardware re-verification (I1, I8).} Not a backlog item so much as
a gate: nothing else in this roadmap should be trusted until the Session 5
geometry fixes are confirmed against a live run and calibration is confirmed
current (Section~\ref{sec:open-issues}).

\paragraph{R1 -- Zone-bounds-aware release placement.}
...
position + size, for \texttt{targets}.} Replace
...
\paragraph{Benchmark extension.} Add a third \texttt{VLM\_LLM} condition to
the existing 120-trial study (Section~\ref{sec:benchmark}), to test whether
hybrid grounding recovers arithmetic-task success rate closer to LLM's
while keeping VLM's live-camera advantage.
