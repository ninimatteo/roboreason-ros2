...
\begin{enumerate}[label=\textbf{I\arabic*.}, leftmargin=1.6cm]
  ...
  \item \textbf{Session 5 geometry fixes are not yet hardware-verified
    end-to-end.} \textbf{Resolved, September 2026:} the 120-trial benchmark
    (Section~\ref{sec:benchmark}) is exactly this end-to-end live run, at
    far more than a handful of prompts -- 98.3\%/85.0\% task safety (LLM/VLM)
    across all 120 trials, with every incident a collision during
    multi-object placement rather than a geometry-fix regression (no wrong
    heights or missed footprints in any of the 22 operator notes).
    \texttt{\_fix\_object\_height} / \texttt{\_fix\_release\_height}
